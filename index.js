...
async function renderKeysUI(i, page = 0) {
  ...
  return i.update
    ? i.update({ embeds: [embed], components: [menu, nav] })
    : i.reply({ embeds: [embed], components: [menu, nav], flags: 64 });
}
  ...
  try {
    if (i.isChatInputCommand()) {
      ...
      if (i.commandName === "getkey") {
        const res = await apiGet(`${API}/api/keys/unused`);
    ...
    if (i.isButton()) {
      ...
      if (id === "back_keys") return renderKeysUI(i, 0);
